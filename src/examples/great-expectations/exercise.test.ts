...
describe('Kanban Board', () => {
  ...
  it('should include an added status in board.statuses using #addStatus', () => {
    ...
    expect.hasAssertions();
    board.addStatus('Pun');
    expect(board.statuses).toContain('Pun');
  });

  it('should remove a status using #removeStatus', () => {
    const board = new KanbanBoard('Things to Do');
    expect.hasAssertions();
    board.addStatus('Clever');
    expect(board.statuses).toContain('Clever');
    board.removeStatus('Clever');
    expect(board.statuses).not.toContain('Clever');
  });
});
...
describe('Person', () => {
  ...
  it('will mutually remove friends', () => {
    ...
    expect(paul).not.toContain(john);
  });
});
